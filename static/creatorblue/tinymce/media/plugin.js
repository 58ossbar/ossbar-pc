...
(function() {
  ...
  var getFlashHtml = function(data) {
    var html = '<object data="' + data.source + '" width="' + data.width + '" height="' + data.height +
  ...
  var getAudioHtml = function(data, audioTemplateCallback) {
    if (audioTemplateCallback) {
      ...
      let str =  '';
      if (data.extractObjects && data.extractObjects !== undefined && data.extractObjects !== '') {
        // str = data.extractObjects;
        let extractObjects = JSON.parse(data.extractObjects);
        for (let key in extractObjects) {
          if (extractObjects[key]) {
            str = str + key + '='+ extractObjects[key] + ';|';
            // str = str + ' ' + key + '="'+ extractObjects[key] +'" ';
          }
        }
      }
      return '<audio controls="controls" src="' + data.source + '" title="'+ str +'" ' +'>' + (data.altsource ? '\n<source src="' + data.altsource +
  ...
  var getVideoHtml = function(data, videoTemplateCallback) {
    if (videoTemplateCallback) {
      ...
      return '<video width="' + data.width + '" height="' + data.height + '"' + (data.poster ? ' poster="' + data.poster +
          '"' : '') + ' controls="controls" controlsList="nofullscreen nodownload noremote footbar" '+ 'title="'+ str +'"' +' >\n' +
  ...
  var dataToHtml = function(editor, dataIn) {
    ...
    if (data.embed) {
      ...
      data.extractObjects = dataIn.extractObjects;
      if (data.type === 'iframe') {
  ...
  var getEmbedHtml = function(editor, data) {
    ...
    return embedHandler ? embedPromise(data, loadedData(editor), embedHandler) : defaultPromise(data, loadedData(
      editor));
  };
    ...
    return function(prop) {
      ...
      var getFromValueFirst = function() {
        return getFromData().bind(function(child) {
          return isObject(child) ? getNonEmptyValue(child).orThunk(getFromMetaData) : getFromMetaData().orThunk(
            function() {
  ...
  var getDimensions = function(data, metaData) {
    let extractKeys = [];
    if (metaData &&  typeOf (metaData) === 'object'  && JSON.stringify(metaData) !== '{}') {
    ...
    get(data, 'dimensions').each(function(dims) {
      each([
        'width',
        'height'
      ].concat(extractKeys), function(prop) {
    ...
    if (metaData &&  typeOf (metaData) === 'object'  && JSON.stringify(metaData) !== '{}' && dimensions && JSON.stringify(dimensions) !== '{}') {
      let extractObjects = {};
      for(let keym in metaData){
        for(let keyd in dimensions){
          if (keym === keyd) {
            extractObjects[keym] = dimensions[keyd];
          }
        }
      }
      dimensions.extractObjects = JSON.stringify(extractObjects);
    }
    return dimensions;
  };
  var unwrap = function(data, sourceInput) {
    ...
    return __assign(__assign(__assign(__assign(__assign({}, get('source')), get('altsource')), get('poster')), get(
      'embed')), getDimensions(data, metaData));
  };
  ...
  var handleError = function(editor) {
    return function(error) {
      var errorMessage = error && error.msg ? 'Media embed handler error: ' + error.msg :
  ...
  var createPreviewIframeNode = function(editor, node) {
    ...
    //莫若卿魔改开始部分
    if (name == 'video') {
		let a = node.firstChild.value;
		var parser = new DOMParser(); 
		var resXML = parser.parseFromString(a,'text/xml');
		let vd = resXML.querySelector('source').getAttribute('src');
      ...
      // 对 额外添加的 属性 进行分解
      if (node.attributes.map && node.attributes.map.title && (node.attributes.map.title).indexOf(';|') > -1 ) {
        let arr = (node.attributes.map.title).split(';|');
      ...
      previewNode.attr(attr);
    }
    if (name == 'audio') {
      let vd = node.attributes.map.src;
      let attr = {
        src: vd,
        controls: '',
        style: 'display:block;margin:auto;max-width:100%;',
    ...
    return function(nodes) {
      ...
      while (i--) {
        ...
        if (videoScript) {
          ...
          node.attr('controlsList', 'nofullscreen nodownload noremote footbar');
        }
  ...
  var setup = function(editor) {
    editor.on('preInit', function() {
      ...
      global$1.each('video audio object'.split(' '), function(name) {
      ...
      editor.parser.addNodeFilter('video,audio,object,embed,script', Nodes.placeHolderConverter(editor));
