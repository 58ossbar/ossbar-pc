...
var InitData_screenShotController = null; // 获取截图工具栏容器dom
...
var screenShotImageController = null; // 数据初始化标识
...
var InitData_InitData = /*#__PURE__*/function () {
  ...
  _createClass(InitData, [{
    ...
    value: function setScreenShotPosition(left, top) {
      this.getScreenShotContainer();
      if (InitData_screenShotController == null) return;
      InitData_screenShotController.style.left = left + "px";
      InitData_screenShotController.style.top = top + "px";
    } // 显示截图区域容器
    ...
    value: function destroyDOM() {
      if (InitData_screenShotController == null || InitData_toolController == null || optionIcoController == null || optionController == null || InitData_textInputController == null || cutBoxSizeContainer == null) return; // 销毁dom
...
function getCanvasImgData(isSave) {
  ...
  var data = new InitData_InitData();
  var screenShotCanvas = (_data$getScreenShotCo = data.getScreenShotContainer()) === null || _data$getScreenShotCo === void 0 ? void 0 : _data$getScreenShotCo.getContext("2d"); // 获取裁剪区域位置信息
...
function toolClickEvent(toolName, index, mouseEvent, completeCallback, closeCallback) {
  ...
  if (screenShotController == null || ScreenShotImageController == null) return; // 获取canvas容器

  var screenShotCanvas = screenShotController.getContext("2d"); // 工具栏尚未点击，当前属于首次点击，重新绘制一个无像素点的裁剪框
...
function drawMasking(context, imgData) {
  ...
  var canvasSize = plugInParameters.getCanvasSize(); // 清除画布
...
var KeyboardEventHandle_KeyboardEventHandle = /*#__PURE__*/function () {
  ...
  function KeyboardEventHandle(screenShotController, toolController) {
    ...
    screenShotController.tabIndex = 9999; // 监听截图容器键盘按下事件
...
function setPlugInParameters(options) {
  ...
  if ((options === null || options === void 0 ? void 0 : options.enableWebRtc) === true) {
...
var main_ScreenShot = /*#__PURE__*/function () {
  ...
  function ScreenShot(options) {
    ...
    _defineProperty(this, "screenShot", function (cancelCallback) {
      ...
      _this.startCapture(cancelCallback).then(function () {
        setTimeout(function () {
          var _this$screenShotConta, _this$screenShotImage, _this$screenShotImage2, _this$screenShotImage3;
          ...
          var containerHeight = (_this$screenShotImage2 = _this.screenShotImageController) === null || _this$screenShotImage2 === void 0 ? void 0 : _this$screenShotImage2.height; // 用户有传宽高时，则使用用户的
          ...
          (_this$screenShotImage3 = _this.screenShotImageController.getContext("2d")) === null || _this$screenShotImage3 === void 0 ? void 0 : _this$screenShotImage3.drawImage(_this.videoController, 0, 0, containerWidth, containerHeight); // 初始化截图容器

          _this.initScreenShot(undefined, context, _this.screenShotImageController); // 停止捕捉屏幕
    ...
    _defineProperty(this, "mouseDownEvent", function (event) {
      ...
      if (_this.data.getToolName() == "text" && _this.textInputController && _this.screenShotContainer && _this.screenShotCanvas) {
        ...
        if (_this.textInputPosition.mouseX != 0 && _this.textInputPosition.mouseY != 0 && _this.textInputPosition.mouseX != mouseX && _this.textInputPosition.mouseY != mouseY) {
          drawText(_this.textInputController.innerText, _this.textInputPosition.mouseX, _this.textInputPosition.mouseY, _this.data.getSelectedColor(), _this.fontSize, _this.screenShotCanvas); // 输入框内容不为空时则隐藏
    ...
    _defineProperty(this, "mouseMoveEvent", function (event) {
      ...
      if (_this.data.getToolClickStatus() && _this.data.getDragging()) {
        ...
        switch (_this.data.getToolName()) {
          case "square":
            drawRectangle(startX, startY, tempWidth, tempHeight, _this.data.getSelectedColor(), _this.data.getPenSize(), _this.screenShotCanvas, _this.screenShotContainer, _this.screenShotImageController);
            break;
      ...
      _this.operatingCutOutBox(currentX, currentY, startX, startY, width, height, _this.screenShotCanvas); // 如果鼠标未点击或者当前操作的是裁剪框都return
    ...
    this.screenShotImageController = document.createElement("canvas"); // 实例化响应式data

    this.data = new InitData_InitData(); // 设置插件的可选参数

    this.setOptionalParameter(options); // 获取截图区域canvas容器(获取的同时也会为InitData中的全局变量赋值)

    this.setGlobalParameter(); // 加载截图组件
  ...
  _createClass(ScreenShot, [{
    ...
    value: function load(triggerCallback, cancelCallback) {
      ...
      this.data.setScreenShotInfo(window.innerWidth, window.innerHeight); // 设置截图容器位置
      ...
      this.screenShotImageController.height = window.innerHeight; // 用户有传宽高则使用用户传进来的
      ...
      if (plugInParameters.getWebRtcStatus()) {
        // 设置为屏幕宽高
        this.data.setScreenShotInfo(window.screen.width, window.screen.height); // 设置为屏幕宽高

        this.screenShotImageController.width = window.screen.width;
        this.screenShotImageController.height = window.screen.height; // 用户有传宽高则使用用户传进来的
      ...
      if (!plugInParameters.getWebRtcStatus()) {
        ...
        if (this.imgSrc != null) {
          ...
          imgContainer.onload = function () {
            var _this2$screenShotImag;

            // 装载截图的dom为null则退出
            if (_this2.screenShotContainer == null) return; // 将用户传递的图片绘制到图片容器里

            (_this2$screenShotImag = _this2.screenShotImageController.getContext("2d")) === null || _this2$screenShotImag === void 0 ? void 0 : _this2$screenShotImag.drawImage(imgContainer, 0, 0, _this2.screenShotImageController.width, _this2.screenShotImageController.height); // 初始化截图容器

            _this2.initScreenShot(triggerCallback, context, _this2.screenShotImageController);
          };
        ...
        html2canvas_default()(this.screenShotDom ? this.screenShotDom : document.body, {
          ...
          _this2.screenShotImageController = canvas; // 初始化截图容器
    ...
    value: function initCropBox(cropBoxInfo) {
      ...
      drawCutOutBox(startX, startY, width, height, this.screenShotCanvas, this.data.getBorderSize(), this.screenShotContainer, this.screenShotImageController); // 保存边框节点信息

      this.cutOutBoxBorderArr = saveBorderArrInfo(this.data.getBorderSize(), this.drawGraphPosition); // 修改鼠标状态为拖动

      this.screenShotContainer.style.cursor = "move"; // 显示截图工具栏

      this.data.setToolStatus(true); // 显示裁剪框尺寸显示容器
    ...
    value: function showToolBar() {
      if (this.toolController == null || this.screenShotContainer == null) return; // 计算截图工具栏位置

      var toolLocation = calculateToolLocation(this.drawGraphPosition, this.toolController.offsetWidth);
      var containerHeight = this.screenShotContainer.height; // 当前截取的是全屏，则修改工具栏的位置到截图容器最底部，防止超出
      ...
      if (toolLocation.mouseY > containerHeight - 64) {
        toolLocation.mouseY -= this.drawGraphPosition.height + 64; // 超出屏幕顶部时
        ...
        this.data.setToolPositionStatus(true); // 隐藏裁剪框尺寸显示容器

        this.data.setCutBoxSizeStatus(false);
      } // 显示并设置截图工具栏位置


      this.data.setToolInfo(toolLocation.mouseX + this.position.left, toolLocation.mouseY + this.position.top); // 设置裁剪框尺寸显示容器位置

      this.data.setCutBoxSizePosition(this.drawGraphPosition.startX, this.drawGraphPosition.startY - 35); // 渲染裁剪框尺寸

      this.data.setCutBoxSize(this.drawGraphPosition.width, this.drawGraphPosition.height); // 状态重置
    ...
    value: function initScreenShot(triggerCallback, context, screenShotContainer) {
      ...
      this.screenShotCanvas = context; // 存储屏幕截图

      this.data.setScreenShotImageController(screenShotContainer); // 绘制蒙层
      ...
      (_this$screenShotConta5 = this.screenShotContainer) === null || _this$screenShotConta5 === void 0 ? void 0 : _this$screenShotConta5.addEventListener("mouseup", this.mouseUpEvent); // 是否初始化裁剪框
